// Objective: separate a finished foreground answer from verified specialist work.
// A. Record actual SDK dispatch and result-collection traffic.
// B. Inspect all terminal statuses rather than only incomplete tasks.
// C. Require successful collected outputs and the documented role subset.
...
public sealed class CompletionEvidenceClient(IChatClient inner, Transcript transcript) : DelegatingChatClient(inner)
{
    ...
    public async Task<bool> VerifyAsync(string prompt, CancellationToken cancellationToken)
    {
        ...
        // B. Invoke the actual session-bound SDK read tools; never infer task health
        // from GetIncompleteTasks, which excludes Failed and Lost terminal statuses.
        ...
        // C. Host reads verify evidence but never substitute for missing main-agent fan-in.

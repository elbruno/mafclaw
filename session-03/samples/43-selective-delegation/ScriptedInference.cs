// Objective: test selective dispatch with explicit scripts through real MAF tool execution.
// A. Replace only model inference with exact classroom prompt-to-role scripts.
// B. Read real SDK task identifiers and completion statuses.
// C. Synthesize only real collected worker outputs, never canned task results.
...
public sealed class ScriptedInference(string agentName, string prompt)
{
        ...
        IReadOnlyList<ChatMessage> messages, ChatOptions? options, CancellationToken cancellationToken)
    {
        ...
        return Task.FromResult(agentName == "MainAgent"
            ? Coordinate(messages, options) : Work(messages, options));
    }
    ...
    private ChatResponse Work(IReadOnlyList<ChatMessage> messages, ChatOptions? options)
    {
        if (_stage++ == 0)
        {
            // A. Invoke the one real AIFunction supplied by this specialist's Harness.
    ...
    private ChatResponse Coordinate(IReadOnlyList<ChatMessage> messages, ChatOptions? options)
    {
        ...
        if (_stage == 1)
        {
            // B. The provider chooses task IDs; the script consumes actual start results.
        ...
        // C. No canned math: these values came through real SDK get_task_results calls.
